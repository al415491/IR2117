...
rclcpp_action::CancelResponse handle_cancel(
  const std::shared_ptr<GoalHandleRadius> goal_handle)
{
  RCLCPP_INFO(rclcpp::get_logger("server"), 
...
void execute(const std::shared_ptr<GoalHandleRadius>);
  ...
  const std::shared_ptr<GoalHandleRadius> goal_handle)
{
  ...
  auto feedback = std::make_shared<Radius::Feedback>();
  auto & sequence = feedback->partial_sequence;
  sequence.push_back(0);
  sequence.push_back(1);
  auto result = std::make_shared<Radius::Result>();
